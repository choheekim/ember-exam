...
module.exports = TestCommand.extend({
  ...
  /**
   * Adds an object which maps a list of module from browser id to config
   * 
   * @param {object} commandOptions
   * @param {object} config
   */
  _addBrowserModuleMappingToConfig: function(commandOptions) {
    ...
    const browserModuleMapping = ((replayBrowser, testMappingObj) => {
      ...
      for (const idx in replayBrowser) {
        ...
        const listOfModule = testMappingObj[browserId];

        if(!listOfModule) {
          throw Error ('browser id doesn\t exist in the file.');
        }

        browserModuleMappingObj[browserId] = listOfModule;
      }
  ...
  _addBrowserSocketEventsToConfig: function(config) {
    ...
    events['after-tests-complete'] = function () {
      const proto = Object.getPrototypeOf(this);
      const fileName = 'test-execution-%TIMESTAMP%.json'.replace('%TIMESTAMP%', Date.now());
      const replayExecution = this.config.progOptions.browserModuleMapping;
