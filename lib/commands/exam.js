...
module.exports = TestCommand.extend({
  ...
  _generateCustomConfigs: function(commandOptions) {
    ...
    if (shouldParallelize && !ids) {
      ...
      config.browserModuleMapping = this._addBrowserModuleMappingToConfig(commandOptions);
  ...
  /**
   * Adds an object which maps a list of modules from browser id to config
   * 
   * @param {object} commandOptions
   * @param {object} config
   */
  _addBrowserModuleMappingToConfig: function(commandOptions) {
    const filePath = commandOptions.replayExecution;
    const replayBrowser = commandOptions.replayBrowser;
    ...
    const browserModuleMapping = ((replayBrowser, testMappingObj) => {
      const browserModuleMappingObj = {};
      for (const idx in replayBrowser) {
        const browserId = replayBrowser[idx];
        const listOfModules = testMappingObj[browserId];

        if(!listOfModules) {
          throw Error ('browser ID provided for replay-browser doesn\'t exist in the specified file path for replay-execution.');
        }

        browserModuleMappingObj[browserId] = listOfModules;
      }
      return browserModuleMapping;
    })(replayBrowser, testMappingObj);
  ...
  _addBrowserSocketEventsToConfig: function(config) {
    let events = config.additionalBrowserSocketEvents || {};
