...
var testServerTask = require('./task/test-server');
var testTask = require('./task/test');
var debug = require('debug')('exam');

const executionMappingObj = {};
...
module.exports = TestCommand.extend({
  ...
  availableOptions: [
    ...
    { name: 'load-balance', type: Number,                       description: 'Loads your test balanced' },
  ...
  init() {
    ...
    this.tasks.TestServerTask = testServerTask;
  },
  ...
  /**
   *
   * Instantiate an executionMappingObj object
   *
   * @param {Object} option
   */
  getExecutionMappigInstance: function(options) {

    if (executionMappingObj !== undefined && executionMappingObj['numberOfBrowsers'] !== undefined && executionMappingObj['executionMapping'] !== undefined) {
      return executionMappingObj;
    }
    const filePath = options.replayExecution;
    const replayBrowser = options.replayBrowser;

    let testMappingObj = {};

    try {
      // Read the replay execution json file.
      const testMappingRawData = fs.readFileSync(filePath);
      testMappingObj = JSON.parse(testMappingRawData);
    } catch (err) {
      const errorMsg = 'Error reading reply execution JSON file - ' + err;
      throw new Error(errorMsg);
    }

    executionMappingObj['numberOfBrowsers'] = testMappingObj['numberOfBrowsers'];
    const browserModuleMappingObj = testMappingObj['executionMapping'];
    executionMappingObj['executionMapping'] = ((replayBrowser, browserModuleMappingObj) => {
      const browserModuleMap = {};
      for (const idx in replayBrowser) {
        const browserId = replayBrowser[idx];
        const listOfModules = browserModuleMappingObj[browserId];

        browserModuleMap[browserId] = listOfModules;
      }
      return browserModuleMap;
    })(replayBrowser, browserModuleMappingObj);
  ...
  run: function(commandOptions) {
    ...
    if(this.validator.shouldLoadBalance) {
      commandOptions.query = addToQuery(commandOptions.query, '_load-balance', commandOptions.loadBalance);
    }
  ...
  _generateCustomConfigs: function(commandOptions) {
    ...
    config.additionalBrowserSocketEvents = this._addBrowserSocketEventsToConfig(config);

    if (!shouldParallelize && !shouldLoadBalance && !shouldReplayExecution) {
      return config;
    }

    let ids = commandOptions.partition;
    let appendingParam = '_partition';

    if (shouldParallelize && !ids) {
      ids = [];
      ...
      appendingParam = '_browser';

      ids = Array.from({length: browserCount}, (v,k) => k+1); // if browser=3 then id = [1,2, 3]
    } else if (shouldReplayExecution) {
      const executionMapping = this.getExecutionMappigInstance(commandOptions);
      config.browserModuleMapping = executionMapping['executionMapping'];

      appendingParam = '_browser';
      ids = commandOptions.replayBrowser;
    }

    if (Array.isArray(customBaseUrl)) {
      const command = this;
      config.testPage = customBaseUrl.reduce(function(testPages, customBaseUrl) {
        return testPages.concat(command._generateTestPages(customBaseUrl, appendingParam, ids));
      }, []);
    } else {
      config.testPage = this._generateTestPages(customBaseUrl, appendingParam, ids);
    }
   ...
   * @param {Object} commandOptions
   * @return {Object}
   * @example tests/index.html?hidepassed&_split=3 if parallel
   *          tests/index.html?hidepassed&_split=3&_load-balance if load-balance
   *          tests/index.html?hidepassed&_split=3&_load-balance&_partition=1&_partition=2 if load-balance and partitions are specified
   */
  _getCustomBaseUrl: function(config, commandOptions) {
    ...
    const appendParamToBaseUrl = function(baseUrl) {
      if(commandOptions.weighted) {
        baseUrl = addToUrl(baseUrl, '_weighted', true)
      }
      if (command.validator.shouldParallelize) {
        baseUrl = addToUrl(baseUrl, '_split', splitCount);
      } else if (command.validator.shouldLoadBalance) {
        baseUrl = addToUrl(baseUrl, '_load-balance', true)
        if (command.validator.shouldSplit) {
          baseUrl = addToUrl(baseUrl, '_split', splitCount);
        }
        const partitions = commandOptions.partition;
        if (partitions) {
          for (let i = 0; i < partitions.length; i++) {
            baseUrl = addToUrl(baseUrl, '_partition', partitions[i]);
          }
        }
      }
   ...
   */
  _generateTestPages: function(customBaseUrl, appendingParam, browserIds) {
    const testPages = [];
  ...
  /**
   * Adds additional event handlers to config.
   * 
   * @param {object} config
   * @return {object} events
   */
  _addBrowserSocketEventsToConfig: function(config) {
    ...
    events['set-modules-queue'] = function (modules) {
      ...
      const browserId = (() => {
        const testPage = this.launcher.settings.test_page;
        return /_browser=\s*(.*)/.exec(testPage)[1];
      })();

      //browserModuleMapping is defined if `replay-execution` option is being used.
      const browserModuleMapping = this.config.progOptions.browserModuleMapping;
      if (browserModuleMapping && !this.module) {
    ...
    events['get-next-module'] = function () {
      const proto = Object.getPrototypeOf(this);
      const replayExecution = this.config.progOptions.browserModuleMapping;
      const moduleQueue = this.moduleQueue;
      const moduleName = moduleQueue.shift();

      if (!moduleName) {
        this.socket.emit('module-queue-complete');
      } else {
        this.socket.emit('get-more-tests-response', moduleName);
      }
      ...
      // if this run is for failure reproduction it's not need to create moduleMap.
      if (!replayExecution) {
    ...
    events['after-tests-complete'] = function () {
      ...
      // if it is failure reproduction run do not create a file.
      if (replayExecution) {
        return;
      }

      if (!proto.finishedBrowser) {
        proto.finishedBrowser = 1;
      }

      const testPages = config.testPage;
      const browserCount = Object.keys(testPages).length;

      if (proto.finishedBrowser == browserCount) {
        const testExecutionJson = JSON.stringify(proto.moduleMap);
        fs.writeFile(fileName, testExecutionJson, (err) => {
          if (err) throw err;
        });
      }
