...
Object.defineProperty(TestsOptionsValidator.prototype, 'shouldParallelize', {
  get: function _getShouldParallelize() {
    ...
    if (typeof this.loadBalance !== 'undefined') {
...
Object.defineProperty(TestsOptionsValidator.prototype, 'shouldLoadBalance', {
  get: function _shouldLoadBalance() {
    ...
    if(!loadBalance) {
      throw new Error('You must specify a \'load-balance\' value in order to use \'load-balance\'.');
    }

    if(isNaN(loadBalance)) {
      throw new Error('You must specify a load-balance value numeric value.')
    }
