 ...
 */
function validateReplyBrowser(replayBrowser, replayExecution) {
  if(!replayExecution) {
    throw new Error('You must specify a \'replay-execution\' path in order to use \'replay-browser\' option.');
  }

  for (const i in replayBrowser) {
    if (replayBrowser[i] < 1) {
      throw new Error('You must specify replay-browser values greater than or equal to 1.');
    }
  }

  validateElementsUnique(replayBrowser, 'browser');
}
...
function validateElementsUnique(arr, typeOfValue) {
  ...
  for (var i = 0; i < arr.length - 1; i++) {
    if (arr[i] === arr[i + 1]) {
      const errorMsg = 'You cannot specify the same %TYPE_OF_VALUE% value twice. %VALUE% is repeated.'
      .replace('%TYPE_OF_VALUE%', typeOfValue)
      .replace('%VALUE%', arr[i].toString());
...
Object.defineProperty(TestsOptionsValidator.prototype, 'shouldReplayExecution', {
  get: function _getShouldReplyExecution() {
    ...
    if(!replayBrowser) {
      throw new Error('You must specifiy the `replay-browser` option in order to use `replay-execution` option.');
    }

    validateReplyBrowser(replayBrowser, replayExecution);
